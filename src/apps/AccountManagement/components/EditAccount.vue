...
<template>
  <div class="edit-account">
    <Row>
      <Col span="12" v-if="!isCurrentEditAccountEmpty">
        <Form ref="accountForm" :model="formData" :rules="formRules" :label-width="80">
          <Form-item label="账号" prop="account">
            <Input v-model="formData.account" placeholder="请输入平台账号"></Input>
          </Form-item>
      ...
      <Col span="24">
        <XNoData :show="isCurrentEditAccountEmpty">暂无要编辑的数据！</XNoData>
      </Col>
    </Row>
  </div>
</template>
  ...
  export default {
    ...
    created: async function () {
      ...
      if (!_t.isCurrentEditAccountEmpty) {
        // TODO 拉取应用列表
        let res = await _t.$store.dispatch('Apps/AccountManagement/Apps/all', {
          noToken: true
        })
        res = _t.$utils.Serv.handleRes(res, true)
        if (!res) {
